...
`tidy` is a logical parameter accepting either TRUE or FALSE, defaulting to TRUE. If TRUE, `hansard` will fix variable names, which by default contain non alpha-numeric characters and appear to use an inconsistent/idiosyncratic naming convention, at least by the standards of the various naming conventions used in R. Dates are converted to Date class, and datetimes to POSIXct class. Some extra URL data included in the API is also stripped out.
...
The search function is not case sensitive, and searchs in the names and constituencies of all MPs and Lords. So even though we spelled her surname incorrectly, we can still find her. This API provides limited biographical details, to retrieve more detailed biographical information, use the [`mnis`](https://cran.r-project.org/package=mnis) package to retrives data from the Members' Names Information Service.
