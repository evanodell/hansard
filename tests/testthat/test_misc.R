...
test_that("papers laid, publications, sessions, tv  functions return expected format", {
    ...
    tvempty <- tv_programmes(start_date = "2016-12-10", end_date = "2017-02-14", tidy = FALSE)
    expect_length(tvempty, 8)
    expect_type(tvempty, "list")
    expect_is(tvempty, "data.frame")
    expect_equal(nrow(tvempty), 395)
    ...
    expect_is(tvclipsempty, "data.frame")
