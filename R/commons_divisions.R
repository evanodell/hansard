...
commons_divisions <- function(comsDivType = c("all", "date", "no",
                                              "aye", "voteSummary",
                                              "voteFull", "session")) {
    ...
    if (comsDivType == "all") {
        ...
        for (i in 0:divisJpage) {
            mydata <- jsonlite::fromJSON(paste0(baseurl_divis, divis_date, ".json?_pageSize=500",
        ...
        df <- jsonlite::rbind.pages(pages[sapply(pages, length) > 0])  #The data frame that is returned
          ...
          for (i in 0:divisJpage) {
            mydata <- jsonlite::fromJSON(paste0(baseurl_divis,
                                                divis_session,
                                                "&_pageSize=500&_page=",
                                                i), flatten = TRUE)
            message("Retrieving page ", i + 1, " of ", divisJpage + 1)
