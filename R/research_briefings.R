...
research_briefings <- function(topic = NULL, subtopic = NULL, type = NULL, extra_args) {
    ...
    if (is.null(topic) == TRUE & is.null(subtopic) == TRUE) {
        ...
        research <- jsonlite::fromJSON(paste0(baseurl, topic_query, subtopic_query, ".json?", query, extra_args), 
            flatten = TRUE)
