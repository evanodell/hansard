...
lwq_multi <- function(answering_department, peer_id, start_date, end_date, extra_args, 
    verbose) {
    ...
    for (i in 1:nrow(search_grid)) {
        ...
        dat[[i]] <- hansard::lords_written_questions(answering_department = search_grid$department[[i]], 
            peer_id = search_grid$member[[i]], end_date = end_date, start_date = start_date, 
...
ldsum_tidy <- function(df, tidy_style) {
    ...
    if (nrow(df) > 0) {
        ...
        df$vote.member <- gsub("http://data.parliament.uk/resources/members/api/lords/id/", 
            "", df$vote.member)
        
        if (tidy_style == "camelCase") {
            
            df$vote.type <- gsub("(^|[^[:alnum:]])([[:alnum:]])", "\\U\\2", df$vote.type, 
                perl = TRUE)
            ...
            if ("amendedDate" %in% colnames(df[i, ]$hasRegisteredInterest[[1]])) 
                {
                  ...
                  df[i, ]$hasRegisteredInterest[[1]]$amendedDate[df[i, ]$hasRegisteredInterest[[1]]$amendedDate == 
                    "NULL"] <- NA  #Change the NULL values to NA to be more R like
