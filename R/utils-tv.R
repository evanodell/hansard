...
tv_tidy <- function(df, tidy_style) {
    ...
    if (nrow(df) > 0) {
        ...
        df$startDate._value <- stringi::stri_replace_all_fixed(df$startDate._value, "T", " ",
                                                               vectorize_all = FALSE)
        ...
        df$endDate._value <- stringi::stri_replace_all_fixed(df$endDate._value, "T", " ",
                                                             vectorize_all = FALSE)
...
tv_tidy2 <- function(df, mp_id, tidy_style) {
    ...
    if (nrow(df) > 0) {
        ...
        if (is.null(mp_id) == FALSE) {
            ...
            df$member_label_value <- stringi::stri_replace_all_fixed(df$member_label_value,
                                                                     "Biography information for ", "",
                                                                     vectorize_all = FALSE)

            df$member_about <- stringi::stri_replace_all_fixed(df$member_about,
                                                               "http://data.parliament.uk/terms/", "",
                                                               vectorize_all = FALSE)
