...
commons_oral_questions_multi <- function(mp_id, answering_department, start_date, 
    ...
    for (i in 1:nrow(search_grid)) {
        ...
        dat[[i]] <- hansard::commons_oral_questions(mp_id = search_grid$member[[i]], 
            answering_department = search_grid$department[[i]], end_date = end_date, 
            start_date = start_date, extra_args = extra_args, verbose = verbose, 
            tidy = FALSE)
...
commons_written_questions_multi <- function(mp_id, answering_department, start_date, 
    end_date, extra_args, verbose) {
...
caq_multi <- function(answering_department, answered_by, start_date, end_date, extra_args, 
    verbose) {
    ...
    for (i in 1:nrow(search_grid)) {
        ...
        dat[[i]] <- hansard::commons_answered_questions(answering_department = search_grid$department[[i]], 
            answered_by = search_grid$member[[i]], end_date = end_date, start_date = start_date, 
            extra_args = extra_args, verbose = verbose, tidy = FALSE)
