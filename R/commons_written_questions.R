...
#' @param answering_department Accepts a string with a department name or partial name, or an array of such strings. The query acts as a search, so entering <health> will return all questions answered by the Department of Health. If \code{NULL}, answering_department is not included as a query parameter. Defaults to \code{NULL}.
