...
#' @param department Requests a Requests a department name, and returns all written questions by department. The query acts as a search, so entering <health> will return all questions answered by the Department of Health
#' @keywords bills
#' @export
#' @examples \dontrun{
...
commons_oral_questions <- function(comsOralType = c("all", "times", "daysTabled", "daysAnswered", "askedBy", "session", "department")) {
    ...
    if (comsOralType == "all") {
        ...
        baseurl_oral <- "http://lda.data.parliament.uk/commonsoralquestion.json?mnisId="
        ...
        oral <- jsonlite::fromJSON(paste0(baseurl_writ, answering.department, "&_pageSize=500"))
        
        oralJpage <- round(writ$result$totalResults/writ$result$itemsPerPage, digits = 0)
