...
#' @export
#' @examples \dontrun{
#' x <- mp_vote_record(530, lobby='all')
#'
#' x <- mp_vote_record(530, lobby='content')
#'
#' x <- mp_vote_record(530, lobby='notcontent')
#' }
...
lord_vote_record <- function(peer_id = NULL, lobby = "all", start_date = "1900-01-01", end_date = Sys.Date()) {
    ...
    if (lobby == "content") {
        ...
        content <- jsonlite::fromJSON(paste0(baseurl, peer_id, "&_pageSize=500", dates), flatten = TRUE)
        ...
        for (i in 0:jpage) {
            mydata <- jsonlite::fromJSON(paste0(baseurl, peer_id, "&_pageSize=500", dates, "&_page=", i), flatten = TRUE)
        ...
        notcontent <- jsonlite::fromJSON(paste0(baseurl, peer_id, "&_pageSize=500", dates), flatten = TRUE)
