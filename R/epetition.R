...
epetition <- function(ID = NULL, by_constituency = FALSE) {
    ...
    if (is.null(ID) == FALSE & is.null(by_constituency) == TRUE) {
        ...
        petition <- jsonlite::fromJSON(paste0(baseurl, ID, by_constituency, ".json?"), flatten = TRUE)
        
        df <- as.data.frame(petition$result$primaryTopic)
        
    } else {
        
        petition <- jsonlite::fromJSON(paste0(baseurl, ID, by_constituency, ".json?&_pageSize=500"), flatten = TRUE)
        ...
        for (i in 0:jpage) {
            mydata <- jsonlite::fromJSON(paste0(baseurl, ID, by_constituency, ".json?&_pageSize=500", "&_page=", i), flatten = TRUE)
