...
mp_vote_record <- function(mp_id = NULL, lobby = "all", start_date = "1900-01-01", end_date = Sys.Date()) {
  ...
  dates <- paste0("&_properties=date&max-date=", end_date, "&min-date=", start_date)
    ...
    if (lobby == "aye") {
      ...
      url_aye <- jsonlite::fromJSON(paste0(baseurl, mp_id, "&_pageSize=500", dates), flatten = TRUE)
      ...
      for (i in 0:jpage) {
        mydata <- jsonlite::fromJSON(paste0(baseurl, mp_id, "&_pageSize=500", dates, "&_page=", i), flatten = TRUE)
      ...
      df$date._value <- as.Date(df$date._value)
        ...
        url_no <- jsonlite::fromJSON(paste0(baseurl, mp_id, "&_pageSize=500", dates), flatten = TRUE)
      ...
      df_aye <- jsonlite::rbind.pages(pages[sapply(pages, length) > 0])  #The data frame that is returned
