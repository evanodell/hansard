...
commons_answered_questions_console <- function(comsAnsType = c("all", "date", "department", "answeredBy")) {
    ...
    if (comsAnsType == "all") {
        ...
        for (i in 0:comAnsweredJpage) {
            mydata <- jsonlite::fromJSON(paste0(baseurl_comAnswered, qAnsweredBy, ".json?_pageSize=500&_page=", i), 
                flatten = TRUE)
            message("Retrieving page ", i + 1, " of ", comAnsweredJpage + 1)
            pages[[i + 1]] <- mydata$result$items
        }
...
commons_divisions_console <- function(comsDivType = c("all", "date", "aye", "no", "voteSummary", "voteFull", "session", 
    "uinSummary", "uinFull")) {
    ...
    if (comsDivType == "all") {
        ...
        divis <- jsonlite::fromJSON(paste0("http://lda.data.parliament.uk/commonsdivisions/no.json?mnisId=", mp.id, 
            "&_pageSize=500"))
        ...
        divis <- jsonlite::fromJSON(paste0("http://lda.data.parliament.uk/commonsdivisions/aye.json?mnisId=", mp.id, 
            "&_pageSize=500"))
...
lords_divisions_console <- function(lordsDivType = c("all", "date", "notContent", "content", "session")) {
    ...
    if (lordsDivType == "all") {
        ...
        divis <- jsonlite::fromJSON(paste0("http://lda.data.parliament.uk/lordsdivisions/content.json?mnisId=", mp.id, 
            "&_pageSize=500"))
        ...
        for (i in 0:divisJpage) {
            mydata <- jsonlite::fromJSON(paste0("http://lda.data.parliament.uk/lordsdivisions/aye.json?mnisId=", mp.id, 
