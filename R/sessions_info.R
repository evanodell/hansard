...
sessions_info <- function(days = FALSE, start_date = "1900-01-01", end_date = Sys.Date(), extra_args = NULL, tidy = TRUE, tidy_style = "snake_case") {
    ...
    if (days == FALSE) {
        dates <- paste0("&_properties=startDate&max-startDate=", as.POSIXct(end_date), "&min-startDate=", as.POSIXct(start_date ))
        query <- NULL
    } else {
        dates <- NULL
        query <- "/days"
    ...
    if (days == TRUE) {
        ...
        df <- df[df$date._value <= as.POSIXct(end_date) & df$date._value >= as.POSIXct(start_date ), ]
    }
