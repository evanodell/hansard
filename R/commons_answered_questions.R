...
commons_answered_questions <- function(answering_department = NULL, answered_by = NULL, extra_args=NULL) {
    ...
    answered <- jsonlite::fromJSON(paste0(baseurl, query, ".json?", answering_department, answered_by, "&_pageSize=500", extra_args), flatten = TRUE)
    ...
    for (i in 0:jpage) {
        mydata <- jsonlite::fromJSON(paste0(baseurl, query, ".json?", answering_department, answered_by, "&_pageSize=500&_page=", i, extra_args), flatten = TRUE)
