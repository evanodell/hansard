...
members <- function(ID = NULL) {
    ...
    members <- jsonlite::fromJSON(paste0(baseurl, ID, query), flatten = TRUE)
    ...
    if (is.null(ID) == TRUE) {
        ...
        for (i in 0:jpage) {
            mydata <- jsonlite::fromJSON(paste0(baseurl, ID, query, "&_page=", i), flatten = TRUE)
...
#' @export
#' @rdname members
commons_members <- function() {
    
    baseurl <- "http://lda.data.parliament.uk/commonsmembers.json?_pageSize=500"
    
    message("Connecting to API")
    
    members <- jsonlite::fromJSON(baseurl, flatten = TRUE)
...
#' @rdname members
#' @export
lords_interests <- function(peer_id = NULL) {
    ...
    members <- jsonlite::fromJSON(paste0(baseurl, query), flatten = TRUE)
