...
edm_search <- function(df, verbose) {
    ...
    df$about <- stringi::stri_replace_all_fixed(df$about, "/signatures/.*",
                                                "", vectorize_all = FALSE)
    ...
    df2$about <- stringi::stri_replace_all_fixed(df2$about, "/signatures/.*", "",
                                                 vectorize_all = FALSE)
...
sig_type <- function(mp_id, primary_sponsor, sponsor,
    ...
    if (primary_sponsor == TRUE) {
        ...
        sig1 <- hansard::mp_edms(mp_id = mp_id, primary_sponsor = TRUE,
    ...
    if (sponsor == TRUE) {
        ...
        sig2 <- hansard::mp_edms(mp_id = mp_id, primary_sponsor = FALSE,
    ...
    if (signatory == TRUE) {
        ...
        sig3 <- hansard::mp_edms(mp_id = mp_id, primary_sponsor = FALSE,
    ...
    df <- df[c("about", "isPrimarySponsor", "isSponsor", "isSignatory",
