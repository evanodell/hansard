...
early_day_motions <- function(edmType = c("all", "allSponsors", "primarySponsor", "signatures", "ID")) {
    ...
    if (edmType == "all") {
     ...
     } else if (edmType=="allSponsors"){#Not tested
          ...
          for (i in 0:edmsJpage) { mydata <- jsonlite::fromJSON(paste0(baseurl_edms,mp.id,"&_pageSize=500", '&_page=', i), flatten = TRUE)
            message('Retrieving page ', i+1, ' of ', edmsJpage+1)
            pages[[i + 1]] <- mydata$result$items
          }
    ...
    } else if (edmType=="primarySponsor"){##Not tested
    ...
    } else if (edmType=="signatures"){##Not tested
