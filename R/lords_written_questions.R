...
#' @export
#' @examples \dontrun{
#' x <- lords_written_questions(peer_id = NULL, answering_department = NULL, start_date = '1900-01-01', end_date = Sys.Date()
#'
#' x <- lords_written_questions(3526, answering_department = 'cabinet')
#'
#' }
...
lords_written_questions <- function(peer_id = NULL, answering_department = NULL, start_date = "1900-01-01", end_date = Sys.Date()) {
    ...
    writ <- jsonlite::fromJSON(paste0(baseurl, query, ".json?", answering_department, peer_id, dates, "&_pageSize=500"), flatten = TRUE)
    ...
    if (jpage == 0) {
        ...
        for (i in 0:jpage) {
            mydata <- jsonlite::fromJSON(paste0(baseurl, query, ".json?", answering_department, peer_id, dates, "&_pageSize=500", i), flatten = TRUE)
